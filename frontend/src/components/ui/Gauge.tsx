...
export function Gauge({ probability, risk, threshold, label = "Fraud probability" }: GaugeProps) {
  ...
  const colour = risk === "HIGH" ? "#F0445C" : risk === "MEDIUM" ? "#F5A524" : "#35C98B";
  ...
  return (
    <div className="flex flex-col items-center" data-testid="risk-gauge">
      ...
      <div className="-mt-10 text-center">
        <div className="text-4xl font-bold tracking-tight text-white" aria-hidden="true">
          {(probability * 100).toFixed(1)}
          <span className="text-xl text-textdim">%</span>
        </div>
        <div className="mt-1 text-xs uppercase tracking-widest text-textdim">{label}</div>
      </div>
    </div>
  );
}
