...
export default function Screening() {
  ...
  const riskColour =
    result?.risk_category === "HIGH" ? "#F0445C" : result?.risk_category === "MEDIUM" ? "#F5A524" : "#35C98B";
      ...
      <div className="grid gap-10 xl:grid-cols-[minmax(0,5fr)_minmax(0,7fr)]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-9" noValidate data-testid="screening-form">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/[0.06] pb-5">
            <div role="tablist" aria-label="Input mode" className="flex rounded-[10px] border border-white/[0.07] bg-surface p-1">
              <button
                type="button"
                role="tab"
                ...
                className={`rounded-lg px-3.5 py-1.5 text-[13px] font-medium transition-colors ${
                  !advanced ? "bg-elevated text-white" : "text-muted hover:text-white"
                }`}
              >
                Quick view
              </button>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {DEMO_SAMPLES.map((s) => (
                <button
                  ...
                  className="rounded-lg border border-white/[0.07] bg-surface px-2.5 py-1 text-xs font-medium text-textdim transition-colors hover:border-accent/40 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50"
                >
                  {s.label}
                </button>
              ))}
          ...
          {loadedSample && (
            <div className="flex items-start gap-2.5 rounded-[10px] border border-accent/20 bg-accent/[0.06] px-3.5 py-3 text-[13px] leading-relaxed text-text">
              ...
              <span>
                <strong className="font-semibold text-white">Demo input loaded.</strong> Values are sample data for
              ...
              <button
                ...
                className="ml-auto shrink-0 rounded p-0.5 text-muted transition-colors hover:text-white"
          ...
          {isSubmitting && stage && (
            <ol className="space-y-2 rounded-[10px] border border-white/[0.06] bg-surface px-4 py-3.5 text-[13px]" aria-label="Analysis progress">
              {STAGES.map((s) => {
                ...
                return (
                  <li key={s.key} className={`flex items-center gap-2.5 ${done ? "text-safe" : active ? "text-white" : "text-muted/50"}`}>
                    {done ? (
                      <CheckCircle2 className="h-3.5 w-3.5" aria-hidden="true" />
                    ) : active ? (
                      <Spinner label="" />
                    ) : (
                      <span className="inline-block h-3.5 w-3.5 rounded-full border border-white/15" aria-hidden="true" />
                    )}
                    {s.label}
                  </li>
                );
        ...
        <div className="space-y-6">
          {!result && (
            <div className="panel-em relative flex min-h-[480px] flex-col items-center justify-center gap-4 px-8 py-16 text-center">
              <span className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/[0.07] bg-[#14181F] text-muted">
                <ShieldCheck className="h-6 w-6" aria-hidden="true" />
              </span>
              <div>
                <h2 className="text-lg font-semibold text-white">Awaiting a screening</h2>
            ...
            <div className="panel-em animate-fadeUp relative overflow-hidden p-7 md:p-9" data-testid="screening-result">
              ...
              <div className="mt-8">
                ...
                <div className="mt-4 flex flex-wrap items-end gap-x-8 gap-y-4">
                  <div className="metric-value text-[3.5rem] leading-none text-[#F5F5F2]">
              ...
              <div className="mt-7">
                <div className="relative h-1.5 rounded-full bg-white/[0.07]">
                  ...
                  />
                  <div
                    className="absolute top-1/2 h-3.5 w-[2px] -translate-y-1/2 rounded-full bg-white/80"
              ...
              <dl className="mt-8 grid grid-cols-3 gap-6 border-t border-white/[0.06] pt-6">
                <div>
                  ...
                  <dd className="mt-1.5 font-mono text-[15px] font-semibold text-white tabular-nums">{formatCurrency(result.amount)}</dd>
                </div>
                <div className="min-w-0">
                  <dt className="label-sm">Reference</dt>
                  <dd className="mt-1.5 truncate font-mono text-[13px] text-white" title={result.external_ref}>{result.external_ref}</dd>
                </div>
                <div>
                  <dt className="label-sm">Screening id</dt>
                  <dd className="mt-1.5 font-mono text-[13px] text-white">{result.screening_id.slice(0, 8)}…</dd>
                </div>
              </dl>

              {/* MODEL SIGNAL vs RULE ENGINE SIGNALS */}
              <div className="mt-9 grid gap-9 border-t border-white/[0.06] pt-7 sm:grid-cols-2">
                <div>
                  ...
                  <p className="mt-3 text-sm text-text">
                    Fraud probability{" "}
                    <span className="font-mono font-semibold text-white tabular-nums">{formatProbability(result.fraud_probability)}</span>
                  </p>
                ...
                {result.explanation.available ? (
                  <>
                    ...
                    <Link
                      ...
                      className="mt-5 inline-flex items-center gap-1.5 text-[13px] font-semibold text-accent transition-colors hover:text-white"
                    >
              ...
              <div className="mt-9 flex flex-wrap items-center justify-between gap-4 border-t border-white/[0.06] pt-6">
...
function EmptyModelState({ message }: { message?: string | null }) {
  return (
    <div className="panel-em flex flex-col items-center gap-4 px-8 py-20 text-center">
      ...
      <h2 className="text-lg font-semibold text-white">Model not available yet</h2>
      <p className="max-w-lg text-sm leading-relaxed text-textdim">{message}</p>
      <code className="rounded-lg border border-white/[0.07] bg-surface px-3.5 py-2 font-mono text-xs text-text">
