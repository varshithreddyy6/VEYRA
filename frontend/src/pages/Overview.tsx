...
const RISK_COLOURS: Record<RiskCategory, string> = { LOW: "#35C98B", MEDIUM: "#F5A524", HIGH: "#F0445C" };
const TOOLTIP_STYLE = {
  background: "#171B23",
...
const AXIS_TICK = { fill: "#747B87", fontSize: 11 };
...
export default function Overview() {
  ...
  return (
    <div className="space-y-12">
      ...
      <section className="flex flex-wrap items-center justify-between gap-x-10 gap-y-4 border-y border-white/[0.06] py-6">
        <div>
      ...
      {model.data && !model.data.available && (
        <div className="flex flex-wrap items-center justify-between gap-4 rounded-[10px] border border-alert/20 bg-alert/[0.07] px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-white">No trained model yet</div>
        ...
        {kpis && (
          <div className="grid gap-x-8 gap-y-9 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Metric
              ...
              icon={Siren}
              dot={alerts.data && alerts.data.total > 0 ? "#F0445C" : "#35C98B"}
              ...
              <AreaChart data={chartData} margin={{ left: -14, right: 4, top: 4, bottom: 0 }}>
                <defs>
                  <linearGradient id="fraudGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#F0445C" stopOpacity={0.22} />
                    <stop offset="100%" stopColor="#F0445C" stopOpacity={0.01} />
                  </linearGradient>
                  <linearGradient id="allGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#848AF2" stopOpacity={0.18} />
                    <stop offset="100%" stopColor="#848AF2" stopOpacity={0.01} />
                  </linearGradient>
                </defs>
                ...
                <Tooltip contentStyle={TOOLTIP_STYLE} labelStyle={{ color: "#F5F5F2", fontWeight: 600 }} />
                <Area type="monotone" dataKey="screenings" name="Screenings" stroke="#848AF2" strokeWidth={1.75} fill="url(#allGrad)" />
                <Area type="monotone" dataKey="fraud" name="Fraud" stroke="#F0445C" strokeWidth={1.75} fill="url(#fraudGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div className="flex h-60 items-center justify-center rounded-[14px] border border-dashed border-white/[0.09] text-sm text-textdim">
            ...
            <>
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={riskData} dataKey="value" nameKey="name" innerRadius={52} outerRadius={78} paddingAngle={3}>
                    {riskData.map((d) => (
                      <Cell key={d.name} fill={RISK_COLOURS[d.name] ?? "#848AF2"} />
                    ))}
                  </Pie>
              ...
              <ul className="mt-4 space-y-2.5">
                {(Object.keys(RISK_COLOURS) as Array<keyof typeof RISK_COLOURS>).map((k) => (
                  <li key={k} className="flex items-center justify-between border-b border-white/[0.04] pb-2.5 text-sm">
                    ...
                    <span className="font-mono tabular-nums text-white">{summary.data?.risk_distribution[k] ?? 0}</span>
                  </li>
                ))}
              </ul>
            </>
      ...
      <section>
        <div className="section-head">
          ...
          <Link to="/alerts" className="inline-flex items-center gap-1 text-[13px] font-semibold text-accent transition-colors hover:text-white">
        ...
        {alerts.data && alerts.data.items.length === 0 && (
          <div className="rounded-[14px] border border-dashed border-white/[0.09] px-6 py-10 text-center text-sm text-textdim">
            ...
            {alerts.data.items.map((a) => (
              <li key={a.id} className="group flex flex-wrap items-center justify-between gap-x-6 gap-y-2 py-4">
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1 text-[13px] font-semibold text-accent transition-colors hover:text-white"
                >
      ...
      <section>
        <div className="section-head">
          ...
          <Link to="/activity" className="inline-flex items-center gap-1 text-[13px] font-semibold text-accent transition-colors hover:text-white">
