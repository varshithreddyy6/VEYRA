...
export default function ModelPerformance() {
  ...
  const curveBars = m
    ? [
        { label: "ROC-AUC", value: m.roc_auc ?? 0, hint: "ranking quality", colour: "#16A34A" },
        { label: "PR-AUC", value: m.pr_auc ?? 0, hint: "rare-event benchmark", colour: "#E03E5E" },
        { label: "Accuracy", value: m.accuracy ?? 0, hint: "context only", colour: "#94A3B8" },
      ]
    : [];
    ...
    <div className="space-y-12">
      ...
      {metrics.data && !metrics.data.available && (
        <div className="panel-em flex flex-col items-center gap-4 px-8 py-20 text-center">
          <span className="flex h-12 w-12 items-center justify-center rounded-2xl border border-border bg-surface text-textdim">
            <GaugeIcon className="h-6 w-6" aria-hidden="true" />
          </span>
          <h2 className="text-lg font-semibold text-text">No metrics available</h2>
      ...
      {metrics.data?.available && m && (
        <>
          ...
          <section className="flex flex-wrap items-center gap-x-8 gap-y-2 border-y border-border py-4">
            <div>
              <div className="label-sm">Model version</div>
              <div className="mt-1 font-mono text-[13px] text-text">{metrics.data.model_version}</div>
            </div>
            <div>
              <div className="label-sm">Dataset</div>
              <div className="mt-1 font-mono text-[13px] text-text">{metrics.data.dataset ?? "—"}</div>
            </div>
            <div>
              <div className="label-sm">Threshold selected by</div>
              <div className="mt-1 font-mono text-[13px] text-text">{metrics.data.threshold?.selected_by ?? "max_f1"}</div>
            </div>
            <div>
          ...
          <section aria-label="Key metrics">
            ...
            <div className="grid gap-x-8 gap-y-9 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <Metric label="Recall" value={formatProbability(m.recall)} dot="#D97706" hint="of the real fraud, how much the model caught." />
              <Metric label="F1 score" value={formatProbability(m.f1)} dot="#16A34A" hint="one score balancing precision and recall." />
              <Metric label="PR-AUC" value={formatProbability(m.pr_auc)} dot="#16A34A" hint="the right benchmark for 0.2% fraud rates." />
              ...
              <dl className="divide-y divide-border">
                {operatingPoints.map((c) => (
                  <div key={c.label} className="flex items-baseline justify-between gap-6 py-3.5">
                    ...
                    <dd className="font-mono text-[15px] font-semibold text-text tabular-nums">{c.value}</dd>
                  </div>
                ))}
              </dl>
            ...
            {cm && (
              <div>
                ...
                <div className="grid grid-cols-[auto_auto_auto] gap-2">
                  <span />
                  <span className="pb-1 text-center text-xs font-medium text-muted">Predicted fraud</span>
                  <span className="pb-1 text-center text-xs font-medium text-muted">Predicted legit</span>
                  ...
                  <div className="rounded-[10px] border border-border bg-surface px-5 py-4 text-center">
                  ...
                  <div className="rounded-[10px] border border-safe/20 bg-safe/[0.05] px-5 py-4 text-center">
                    <div className="font-mono text-2xl font-semibold text-text tabular-nums">{cm.true_negative.toLocaleString()}</div>
            ...
            <div>
              ...
              <dl className="divide-y divide-border">
                <div className="flex justify-between gap-6 py-3">
                  ...
                  <dd className="font-mono text-[13px] text-text">{metrics.data.model_version}</dd>
                </div>
                <div className="flex justify-between gap-6 py-3">
                  <dt className="text-sm text-textdim">Dataset</dt>
                  <dd className="font-mono text-[13px] text-text">{metrics.data.dataset ?? "—"}</dd>
                </div>
                <div className="flex justify-between gap-6 py-3">
                  <dt className="text-sm text-textdim">Fraud count (test)</dt>
                  <dd className="font-mono text-[13px] text-text">{m.fraud_count?.toLocaleString() ?? "—"}</dd>
                </div>
                <div className="flex justify-between gap-6 py-3">
                  <dt className="text-sm text-textdim">Legit count (test)</dt>
                  <dd className="font-mono text-[13px] text-text">{m.legit_count?.toLocaleString() ?? "—"}</dd>
                </div>
              </dl>
            </div>
