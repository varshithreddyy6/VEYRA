        ...
        <section data-testid="batch-upload">
          ...
          <div
            ...
            className={`flex h-64 cursor-pointer flex-col items-center justify-center gap-4 rounded-card border border-dashed px-6 text-center transition-colors duration-200 ${
              dragOver ? "border-accent/60 bg-accent/[0.06]" : "border-white/[0.14] bg-white/[0.015] hover:border-white/25 hover:bg-white/[0.025]"
            }`}
          >
            <span className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] text-accent">
              <UploadCloud className="h-5 w-5" aria-hidden="true" />
            </span>
            <div className="text-[15px] font-medium text-white">Drop your CSV here, or click to browse</div>
          ...
          {file && (
            <div className="mt-5 flex items-center justify-between rounded-[10px] border border-safe/20 bg-safe/[0.05] px-4 py-3">
              ...
              <button type="button" onClick={() => setFile(null)} className="rounded p-1 text-textdim transition-colors hover:text-white" aria-label="Remove file">
        ...
        <section>
          ...
          {job && (
            <div className="panel-em space-y-6 p-7" data-testid="batch-job-status">
                      ...
                      <li key={s.key} className="flex flex-1 items-center gap-2">
                        <span
                          className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[10px] font-semibold ${
                            reached ? "border-accent/50 bg-accent/15 text-accent" : "border-white/10 bg-white/[0.03] text-textdim"
                          }`}
                          aria-hidden="true"
                        >
                          {reached && i < stepIndex ? "✓" : i + 1}
                        </span>
                        <span className={`text-2xs ${isCurrent ? "font-semibold text-white" : "text-textdim"}`}>{s.label}</span>
                        {i < STATUS_STEPS.length - 1 && <span className="h-px flex-1 bg-white/[0.07]" aria-hidden="true" />}
                      </li>
                    );
                  })}
                </ol>
              ...
              {job.status === "done" && (
                <>
                  <div className="grid grid-cols-2 gap-6 border-y border-white/[0.06] py-5">
                    <div>
          ...
          {job?.preview && job.status === "done" && (
            <details className="mt-5">
              <summary className="cursor-pointer text-[13px] font-semibold text-accent transition-colors hover:text-white">
                Preview result rows
              </summary>
              <div className="mt-3 max-h-72 overflow-auto rounded-card border border-white/[0.06]">
                <table className="data-table min-w-[460px]">
                  <thead className="sticky top-0 bg-[#181A2B]">
                    <tr>
                  ...
                  <tbody>
                    {job.preview.slice(0, 10).map((row, i) => (
                      <tr key={i}>
                        ...
                        <td className="num px-4 text-right text-white">${String(row.amount ?? "")}</td>
                        <td className="num px-4 text-right text-white">{formatProbability(Number(row.fraud_probability ?? 0))}</td>
                        <td className="px-4">
            ...
            {jobs.data.items.map((j) => (
              <li key={j.id} className="flex flex-wrap items-center justify-between gap-x-6 gap-y-2 py-4">
                <div className="min-w-0">
                  <div className="truncate text-sm font-medium text-white">{j.filename}</div>
