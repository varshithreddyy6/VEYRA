  ...
  return <div className="home-shell min-h-screen">
    <header className="home-nav">
      <div className="home-nav-inner">
        ...
        <div className="flex items-center gap-2 sm:gap-3"><span className="hidden text-sm text-textdim sm:inline">{user?.full_name}</span><button type="button" className="home-signout" onClick={async () => { await logout(); navigate("/login"); }}>Sign out</button></div>
      </div>
    </header>
    <main className="home-content">
      <div className="home-intro"><div className="eyebrow">Your workspace</div><h1>Good to see you, {firstName}</h1><p>Choose a workspace to begin.</p></div>
      <section className="bento-grid" aria-label="Feature workspaces">
        {tiles.map(({ to, title, description, icon: Icon, className }) => <Link key={to} to={to} className={`bento-tile ${className}`}><div className="bento-icon"><Icon className="h-5 w-5" aria-hidden="true" /></div><div className="mt-auto pt-8"><div className="flex items-end justify-between gap-3"><h2>{title}</h2><ArrowUpRight className="bento-arrow h-5 w-5" aria-hidden="true" /></div><p>{description}</p></div></Link>)}
      </section>
    </main>
  </div>;
}
